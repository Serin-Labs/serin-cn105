...
Land this repo's validator, `scripts/requirements-validation.txt` and pinned
public key on `main` first. Then land the HomeKit and Matter
...
The encrypted pre-0.1.7 feeds at `firmware/link/manifest.json` and
`firmware/link/beta/manifest.json` were removed on 2026-09-20; firmware that
polled them never reached production. The validator rejects a Link OTA manifest
anywhere but the two `ota/` channel directories, rejects `enc_size`, and checks
that channel metadata matches the directory.
...
replace saved URL overrides. A bench dial still running an encrypted pre-0.1.7
build moves forward with a USB factory install. Stage and validate factory
images separately.
...
| Link factory | `board` (`viewe15`/`viewe21`), `path`, `size`, `sha256`; optional `parts` of `{path, offset, sha256}` | Actual size and hash; bootloader image checksums; partition-table checksum and exact supported layout; signed app at `0x20000` with matching version. With `parts`: exactly the offsets `0x0`, `0x8000`, `0xf000`, `0x20000`; SHA-256 of every part; no part reaches past its region; the `0xf000` part is a full blank otadata partition; the parts plus `0xFF` fill equal the merged image |

Factory manifests live at `firmware/link/factory-manifest.json` (stable) and
`firmware/link/factory/beta/factory-manifest.json` (`channel: "beta"`, pre-release
version); any other directory is rejected. Split parts exist so a USB reinstall
can leave NVS at `0x9000` alone: the merged image pads across it, the parts do
not. Blank otadata forces the bootloader to `ota_0`, so the new app runs even on
a dial whose last OTA left it booting `ota_1`.
